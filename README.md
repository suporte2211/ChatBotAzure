...
const API_URL = env.API_URL || 'https://api.wapp.marketing/v1'
...
app.get('/', (req, res) => {
  res.send({
    name: 'chatbot',
    description: 'Simple WhatsApp chatbot for WApp Marketing',
    endpoints: {
      webhook: {
        path: '/webhook',
        method: 'POST'
...
app.get('/sample', (req, res) => {
  ...
  const data = {
    ...
    message: message || 'Hello World from WApp Marketing!',
...
async function processMessage ({ data, device } = {}) {
  ...
  if (+body === 1 || body === 'reminder-create' || /create|create reminder/i.test(body)) {
    ...
    return await reply({
      ...
      footer: 'Powered by WApp Marketing',
      buttons: [
        {
          text: '1 hour'
        },
...
// Function to send a message using the WApp Marketing API
...
// Find an active WhatsApp device connected to the WApp Marketing API
async function loadDevice () {
  ...
  if (config.device && !config.device.includes(' ')) {
    if (/^[a-f0-9]{24}$/i.test(config.device) === false) {
      return exit('Invalid WhatsApp device ID: must be 24 characers hexadecimal value. Get the device ID here: https://app.wapp.marketing/number')
    }
...
async function main () {
  ...
  if (!config.apiKey || config.apiKey.length < 60) {
    return exit('Please sign up in WApp Marketing and obtain your API key here:\nhttps://app.wapp.marketing/apikeys')
  }
  ...
  // Find a WhatsApp number connected to the WApp Marketing API
  const device = await loadDevice()
  if (!device) {
    return exit('No active WhatsApp numbers in your account. Please connect a WhatsApp number in your WApp Marketing account:\nhttps://app.wapp.marketing/create')
  }
  if (device.session.status !== 'online') {
    return exit(`WhatsApp number (${device.alias}) is not online. Please make sure the WhatsApp number in your WApp Marketing account is properly connected:\nhttps://app.wapp.marketing/${device.id}/scan`)
  }
  if (device.billing.subscription.product !== 'io') {
    return exit(`WhatsApp number plan (${device.alias}) does not support inbound messages. Please upgrade the plan here:\nhttps://app.wapp.marketing/${device.id}/plan?product=io`)
  }
